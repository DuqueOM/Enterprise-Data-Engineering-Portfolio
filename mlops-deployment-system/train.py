...
def train_model(
    ...
    for epoch in range(config.epochs):
        ...
        if epoch > 0 and epoch % 10 == 0:
            ...
            logger.info(
                f"Epoch {epoch}/{config.epochs} - Train accuracy: {train_acc:.4f}"
            )
...
def save_artifacts(
    model: LogisticRegression, metrics: TrainingMetrics, config: TrainingConfig
) -> dict[str, str]:
    """
...
def log_to_wandb(
    config: TrainingConfig, metrics: TrainingMetrics, model: LogisticRegression
) -> None:
    """
    ...
    try:
        with mlflow.start_run(
            ...
            mlflow.sklearn.log_model(
                model, "model", registered_model_name=config.model_name
            )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--batch_size", type=int, default=16, help="Batch size (unused for sklearn)"
    )
    ...
    parser.add_argument(
        "--n_features", type=int, default=DEFAULT_N_FEATURES, help="Number of features"
    )
