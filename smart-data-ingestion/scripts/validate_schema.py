...
from pandera import Check, Column, DataFrameModel
...
def validate_jsonl_file(file_path):
    ...
    if records:
        ...
        logger.info(
            f"   - Average text length: {df['text'].str.len().mean():.1f} chars"
        )
        ...
        if short_texts > 0:
            logger.warning(
                f"⚠️  Found {short_texts} very short text entries (< 50 chars)"
            )
