...
def extract_text_features(df):
    ...
    features['has_phone'] = df['text'].str.contains(r'(\+\d{1,3}[-.]?)?\d{3}[-.]?\d{3}[-.]?\d{4}').astype(int)
    
    # Question/answer indicators
    features['has_question'] = df['text'].str.contains(r'\?').astype(int)
    features['starts_with_question'] = df['text'].str.startswith(r'¿|qué|cómo|dónde|cuándo|por qué').astype(int)
...
def save_model_artifacts(model_data, output_dir):
    ...
    metrics = {
        ...
        'feature_count': len(model_data['feature_columns']) + model_data['vectorizer'].max_features,
...
def main():
    ...
    logger.info(f"   - Top features: {[feat[0] for feat in metrics['top_features'][:5]]}")
