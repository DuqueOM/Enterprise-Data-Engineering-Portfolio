...
def main():
    ...
    parser.add_argument("--slack_webhook", type=str, default=os.environ.get("SLACK_WEBHOOK"))
    parser.add_argument("--github_token", type=str, default=os.environ.get("GITHUB_TOKEN"))
    parser.add_argument("--github_repo", type=str, default=os.environ.get("GITHUB_REPO"))
    ...
    with open(args.input_texts, "r", encoding="utf-8") as f:
        new_texts = [l.strip() for l in f if l.strip()]
    ...
    try:
        ...
        wandb.log({"embedding_drift": drift, "mean_similarity": mean_sim, "n_new": len(new_texts)})
        wandb.finish()
    except Exception as e:
    ...
    if drift > args.threshold:
        ...
        if args.github_token and args.github_repo:
            ...
            create_github_issue(args.github_token, args.github_repo, "DRIFT ALERT", body)
        sys.exit(3)
    else:
        logger.info("No drift detected (drift=%.4f <= threshold=%.4f)", drift, args.threshold)
        sys.exit(0)
